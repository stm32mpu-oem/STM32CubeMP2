  ...
  ======================================================================================================================
    ...
    *** Driver features ***
    ...
    [..]
      This driver provides services covering the LPBAM management of the following SPI features :
          (+) Configure the SPI transfers in simplex or full duplex.
          (+) Starts the SPI data transfers in simplex or full duplex.
          (+) Configure and starts the SPI data transfers in simplex or full duplex.
          (+) The half-duplex is not supported by this driver.
          (+) Configuring the SPI mode (Master/Slave) is not supported by this driver.
    ...
    *** Functional description ***
    ...
    [..]
      ...
      be two queues to be executed by two DMA channels for full duplex operations.
    ...
    *** Driver APIs description ***
    ...
    [..]
      The following parameters must be configured carefully as they are not reconfigurable by LPBAM APIs :
    ...
    *** Driver user sequence ***
      ...
      Generic user sequence is :
          ...
          (+) There are two possibilities to call advanced API:
              ...
              (++) Two APIs making configuration and data setup separately:
                   (+++) Call, when needed, ADV_LPBAM_SPI_Tx_SetConfigQ() and/or ADV_LPBAM_SPI_Tx_SetDataQ(). (Mandatory)
                   (+++) Call, when needed, ADV_LPBAM_SPI_Rx_SetConfigQ() and/or ADV_LPBAM_SPI_Rx_SetDataQ(). (Mandatory)
                   (+++) Call, when needed, ADV_LPBAM_SPI_TxRx_SetConfigQ() and/or ADV_LPBAM_SPI_TxRx_SetDataQ(). (Mandatory)
    ...
    [..]
      ...
      When reconfiguration is needed, ensure using ADV_LPBAM_SPI_TxRx_SetConfigQ() only one time before data transfer
      APIs in the Tx queue.
    ...
    *** Recommendation ***
    ...
    [..]
      ...
      it's mandatory to ensure that IOs are configured to support TxRx TransmitReceive mode.
