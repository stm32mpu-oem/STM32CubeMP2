    ...
    *** Driver user sequence ***
    ...
    [..]
      PWM generation user sequence is :
          ...
          (+) Call ADV_LPBAM_LPTIM_PWM_SetFullQ() to update PWM previous parameters. (Mandatory)
    ...
    [..]
      Input Capture user sequence is :
          ...
          (+) Call ADV_LPBAM_LPTIM_IC_SetFullQ() to measure the Input Capture signal. (Mandatory)
          (+) Call ADV_LPBAM_LPTIM_Stop_SetFullQ().
    ...
    *** Recommendation ***
    ...
    [..]
      ...
      Case 3: The LPTIM peripheral is configured to generate an Update Event:
          (+) It's forbidden to use the LPTIM channel in PWM update mode.
          (+) It's forbidden to use the LPTIM channel in IC mode.
