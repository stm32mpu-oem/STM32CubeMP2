...
static void MX_UART5_Init(void)
{
  ...
  */
  ...
  /* GPIOG clock enable */
  ResMgr_LL_RCC_GPIOG_EnableClock();

  /* GPIOG GPIO get resource */
  if(ResMgr_Request(RESMGR_RESOURCE_RIF_GPIOG, RESMGR_GPIO_PIN(10)) != RESMGR_STATUS_ACCESS_OK &&
		  ResMgr_Request(RESMGR_RESOURCE_RIF_GPIOG, RESMGR_GPIO_PIN(9)) != RESMGR_STATUS_ACCESS_OK) {
	      loc_printf("GPIO PIN 9-10 get resource failed\n");
		  Error_Handler();
  }
  ...
  */
  ...
  if(ResMgr_Request(RESMGR_RESOURCE_RIF_RCC, RESMGR_RCC_RESOURCE(9)) == RESMGR_STATUS_ACCESS_OK) {
...
{
  /* Disable TCPP_EN */
  HAL_GPIO_WritePin(TCPP0203_PORT0_EN_GPIO_PORT, TCPP0203_PORT0_EN_GPIO_PIN, GPIO_PIN_RESET);
  /* DeInit LED */
