...
void HAL_I3C_MspDeInit(I3C_HandleTypeDef* hi3c)
{
  ...
  /* USER CODE END I3C4_MspDeInit 0 */
    ...
    */
	...
	if (RESMGR_STATUS_ACCESS_OK != ResMgr_Release(RESMGR_RESOURCE_RIF_HPDMA3, RESMGR_HPDMA_CHANNEL(0)))
	{
		Error_Handler();
	}
	/* Release HPDMA3 Channel 1 using Resource manager */
	if (RESMGR_STATUS_ACCESS_OK != ResMgr_Release(RESMGR_RESOURCE_RIF_HPDMA3, RESMGR_HPDMA_CHANNEL(1)))
	{
		Error_Handler();
	}
	...
	if (RESMGR_STATUS_ACCESS_OK != ResMgr_Release(RESMGR_RESOURCE_RIFSC, STM32MP25_RIFSC_I3C4_ID))
	{
		Error_Handler();
	}
