...
void HAL_SPI_MspDeInit(SPI_HandleTypeDef* hspi)
{
  ...
  if(SPI3 == hspi->Instance)
  {
    ...
    */
	...
	if (RESMGR_STATUS_ACCESS_OK != ResMgr_Release(RESMGR_RESOURCE_RIFSC, STM32MP25_RIFSC_SPI8_ID))
	{
		Error_Handler();
	}
	/* Release GPIOB10 using Resource manager */
	if (RESMGR_STATUS_ACCESS_OK != ResMgr_Release(RESMGR_RESOURCE_RIF_GPIOB, RESMGR_GPIO_PIN(10)))
	{
		Error_Handler();
	}
	/* Release GPIOB8 using Resource manager */
	if (RESMGR_STATUS_ACCESS_OK != ResMgr_Release(RESMGR_RESOURCE_RIF_GPIOB, RESMGR_GPIO_PIN(8)))
	{
		Error_Handler();
	}
	/* Release GPIOB7 using Resource manager */
	if (RESMGR_STATUS_ACCESS_OK != ResMgr_Release(RESMGR_RESOURCE_RIF_GPIOB, RESMGR_GPIO_PIN(7)))
	{
		Error_Handler();
	}
	/* Release GPIOB1 using Resource manager */
	if (RESMGR_STATUS_ACCESS_OK != ResMgr_Release(RESMGR_RESOURCE_RIF_GPIOB, RESMGR_GPIO_PIN(1)))
	{
		Error_Handler();
	}
