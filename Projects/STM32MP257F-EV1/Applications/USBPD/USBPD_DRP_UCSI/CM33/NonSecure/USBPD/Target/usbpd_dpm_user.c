...
/* USER CODE BEGIN USBPD_DPM_UserInit */
  ...
  {
      /* Start PPM task */
      if (USBPD_OK != USBPD_UCSI_Init(MemoryPtr))
      {
         return USBPD_ERROR;
      }
  }
...
void USBPD_DPM_SNK_EvaluateCapabilities(uint8_t PortNum, uint32_t *PtrRequestData, USBPD_CORE_PDO_Type_TypeDef *PtrPowerObjectType)
{
  ...
  USBPD_DPM_SNKPowerRequestDetails_TypeDef snkpowerrequestdetails = {0};
...
static uint32_t DPM_FindVoltageIndex(uint32_t PortNum, USBPD_DPM_SNKPowerRequestDetails_TypeDef* PtrRequestPowerDetails)
{
  ...
  if (curr_index != DPM_NO_SRC_PDO_FOUND)
  {
    ...
    PtrRequestPowerDetails->OperatingCurrentInmAunits    = (1000U * maxpower) / voltage;
                                     ...
                                     USBPD_SNKRDO_TypeDef* Rdo, USBPD_CORE_PDO_Type_TypeDef *PtrPowerObject)
{
  ...
  rdo.GenericRDO.NoUSBSuspend                 = 1U;
