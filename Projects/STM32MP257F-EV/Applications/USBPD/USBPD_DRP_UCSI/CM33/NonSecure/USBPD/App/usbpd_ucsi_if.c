...
{
  /* Set alert signal */
  VIRT_INTC_sendIRQ(&hintc, ALERT_IRQ_NBR);
  UCSI_IF_PPM_TRACE(UCSI_DEBUG_LEVEL_2, I2C_ADDRESS_UCSI, UCSI_TRACE_ALERT_ON, NULL, 0U);
}
...
{
  /* Reset alert signal */
  UCSI_IF_PPM_TRACE(UCSI_DEBUG_LEVEL_2, I2C_ADDRESS_UCSI, UCSI_TRACE_ALERT_OFF, NULL, 0U);
}
...
#warning "NRTOS Version not implemented"
  ...
  if (USBPD_FAIL != _retr)
  {
    ...
    if (ptr_port->Error == UCSI_ERROR_SEQ_TRANSMIT)
    {
      ...
      UCSI_IF_PPM_DEBUG_TRACE_NO_ARG(UCSI_DEBUG_LEVEL_2, "State UCSI_ERROR_SEQ_TRANSMIT: re-init Virt I2C");
...
static void VIRT_I2C_RxCpltCallback(VIRT_I2C_HandleTypeDef *hi2c, size_t len)
{
  ...
  {
//    UCSI_IF_PPM_DEBUG_TRACE(UCSI_DEBUG_LEVEL_2,
//                            "Msg received on VIRT_I2C (addr:%#x data count:%lu buf[0] = %#x)",
//                            addr, hi2c->msg->len, hi2c->msg->buf[0]);
