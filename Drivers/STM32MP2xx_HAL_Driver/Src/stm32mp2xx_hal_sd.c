                                    ...
                                    uint32_t Timeout)
{
  ...
  if (hsd->State == HAL_SD_STATE_READY)
  {
    ...
    {
      if (__HAL_SD_GET_FLAG(hsd, SDMMC_FLAG_RXFIFOHF) && (dataremaining >= 512U))
      {
                                     ...
                                     uint32_t NumberOfBlocks, uint32_t Timeout)
{
  ...
  if (hsd->State == HAL_SD_STATE_READY)
  {
    ...
    while (!__HAL_SD_GET_FLAG(hsd, SDMMC_FLAG_TXUNDERR | SDMMC_FLAG_DCRCFAIL | SDMMC_FLAG_DTIMEOUT |
                              SDMMC_FLAG_DATAEND))
    {
      if (__HAL_SD_GET_FLAG(hsd, SDMMC_FLAG_TXFIFOHE) && (dataremaining >= 512U))
      {
...
HAL_StatusTypeDef HAL_SD_Abort(SD_HandleTypeDef *hsd)
{
  ...
  if (hsd->State == HAL_SD_STATE_BUSY)
  {
    ...
    hsd->ErrorCode = SDMMC_CmdStopTransfer(hsd->Instance);
    if ((hsd->ErrorCode != HAL_SD_ERROR_NONE)&&(hsd->ErrorCode != HAL_SD_ERROR_CMD_RSP_TIMEOUT))
    {
      return HAL_ERROR;
    }
  
    if ((hsd->Instance->DCTRL & 0x2) == SDMMC_TRANSFER_DIR_TO_CARD)
    {
      if (hsd->ErrorCode == HAL_SD_ERROR_NONE)
      {
        while((!__HAL_SD_GET_FLAG(hsd, SDMMC_FLAG_DABORT))&&(!__HAL_SD_GET_FLAG(hsd, SDMMC_FLAG_BUSYD0END)));
      }
    ...
    else if ((hsd->Instance->DCTRL & 0x2) == SDMMC_TRANSFER_DIR_TO_SDMMC)
    {
      while((!__HAL_SD_GET_FLAG(hsd, SDMMC_FLAG_DABORT))&&(!__HAL_SD_GET_FLAG(hsd, SDMMC_FLAG_DATAEND)));
    }

    /*The reason of all theses while conditions previously is that we need to wait the SDMMC and clear 
...
static uint32_t SD_SendSDStatus(SD_HandleTypeDef *hsd, uint32_t *pSDstatus)
{
  ...
  while (!__HAL_SD_GET_FLAG(hsd, SDMMC_FLAG_RXOVERR | SDMMC_FLAG_DCRCFAIL | SDMMC_FLAG_DTIMEOUT | SDMMC_FLAG_DATAEND))
  {
    if ((!__HAL_SD_GET_FLAG(hsd, SDMMC_FLAG_RXFIFOE)) && (count < 16))
    {
