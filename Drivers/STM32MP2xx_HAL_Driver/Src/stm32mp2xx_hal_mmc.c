                                     ...
                                     uint32_t Timeout)
{
  ...
  if (hmmc->State == HAL_MMC_STATE_READY)
  {
    ...
    {
      if (__HAL_MMC_GET_FLAG(hmmc, SDMMC_FLAG_RXFIFOHF) && (dataremaining >= 512U))
      {
                                      ...
                                      uint32_t NumberOfBlocks, uint32_t Timeout)
{
  ...
  if (hmmc->State == HAL_MMC_STATE_READY)
  {
    ...
    {
      if (__HAL_MMC_GET_FLAG(hmmc, SDMMC_FLAG_TXFIFOHE) && (dataremaining >= 512U))
      {
...
HAL_StatusTypeDef HAL_MMC_Abort(MMC_HandleTypeDef *hmmc)
{
  ...
  if (hmmc->State == HAL_MMC_STATE_BUSY)
  {
    ...
    hmmc->ErrorCode = SDMMC_CmdStopTransfer(hmmc->Instance);
    if ((hmmc->ErrorCode != HAL_MMC_ERROR_NONE)&&(hmmc->ErrorCode != HAL_MMC_ERROR_CMD_RSP_TIMEOUT))
    {
      return HAL_ERROR;
    }
    ...
    if ((hmmc->Instance->DCTRL & SDMMC_DCTRL_DTDIR) == SDMMC_TRANSFER_DIR_TO_CARD)
    {
      if (hmmc->ErrorCode == HAL_MMC_ERROR_NONE)
      {
        while((!__HAL_MMC_GET_FLAG(hmmc, SDMMC_FLAG_DABORT))&&(!__HAL_MMC_GET_FLAG(hmmc, SDMMC_FLAG_BUSYD0END)));
      }
    ...
    {
      while((!__HAL_MMC_GET_FLAG(hmmc, SDMMC_FLAG_DABORT))&&(!__HAL_MMC_GET_FLAG(hmmc, SDMMC_FLAG_DATAEND)));
    }

    /*The reason of all theses while conditions previously is that we need to wait the SDMMC and clear the appropriate flags that will be set depending of the abort/non abort of the memory */
